...
// Верификация SMS кода и регистрация
...
router.post('/login', async (req, res) => {
  ...
  if (!rows.length) return res.status(400).json({ error: 'Неверный номер или пароль' });

  const user = rows[0];
  const ok = await bcrypt.compare(password, user.password);
  if (!ok) return res.status(400).json({ error: 'Неверный номер или пароль' });
